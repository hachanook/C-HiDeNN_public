...
def get_quad_points(Gauss_num, dim, elem_type, device_main):

    # Quad elements
    if elem_type == 'CPE4' or elem_type == 'CPE8' or elem_type.startswith('C3D8') or elem_type.startswith('C3D20'):
        ...
        quad_points = jnp.array(quad_points, dtype=jnp.float64, device=device_main) # (quad_degree*dim, dim)
        quad_weights = jnp.array(quad_weights, dtype=jnp.float64, device=device_main) # (quad_degree,)
        
    # Triangular elements
    elif (elem_type == 'CPE3' or elem_type == 'CPE6' or elem_type.startswith('C3D4') or elem_type.startswith('C3D10')) and dim == 2:
        if Gauss_num == 1:
            quad_weights = jnp.array([1.000000000000000], dtype = jnp.float64, device=device_main)
            quad_points = jnp.array([[0.333333333333333, 0.333333333333333]], dtype = jnp.float64, device=device_main)
        
        elif Gauss_num == 3:
            quad_weights = jnp.array([0.333333333333333, 0.333333333333333, 0.333333333333333], dtype = jnp.float64, device=device_main)
            quad_points = jnp.array([[0.666666666666667, 0.166666666666667],
                                    [0.166666666666667, 0.666666666666667],
                                    [0.166666666666667, 0.166666666666667]], dtype = jnp.float64, device=device_main)
           
        elif Gauss_num == 4:
            quad_weights= jnp.array([-0.562500000000000, 0.520833333333333, 
                                                    0.520833333333333, 0.520833333333333], dtype = jnp.float64, device=device_main)
            quad_points = jnp.array([[0.333333333333333, 0.333333333333333],
                                    ...
                                    [0.200000000000000, 0.200000000000000]], dtype = jnp.float64, device=device_main)

        elif Gauss_num == 6:
            quad_weights = jnp.array([0.109951743655322, 0.109951743655322, 0.109951743655322, 
                                     0.223381589678011, 0.223381589678011, 0.223381589678011], dtype = jnp.float64, device=device_main)
            quad_points = jnp.array([[0.816847572980459, 0.091576213509771],
                                    [0.091576213509771, 0.816847572980459],
                                    [0.091576213509771, 0.091576213509771],
                                    [0.108103018168070, 0.445948490915965],
                                    [0.445948490915965, 0.108103018168070],
                                    [0.445948490915965, 0.445948490915965]], dtype = jnp.float64, device=device_main)
    ...
    # Tetrahedral elements
        ...
        if Gauss_num == 1:
            quad_weights = jnp.array([1.000000000000000], dtype = jnp.float64, device=device_main)
            quad_points = jnp.array([[0.250000000000000, 0.250000000000000, 0.250000000000000]], dtype = jnp.float64, device=device_main)
        
        elif Gauss_num == 4:
            # quad_weights = jnp.array([0.041666666666667, 0.041666666666667, 0.041666666666667, 0.041666666666667], dtype = jnp.float64)
            quad_weights = jnp.array([0.250000000000000, 0.250000000000000, 
                                                     0.250000000000000, 0.250000000000000], dtype = jnp.float64, device=device_main)
            quad_points = jnp.array([[0.585410196624968, 0.138196601125010, 0.138196601125010],
                                     ...
                                     [0.138196601125010, 0.138196601125010, 0.138196601125010]], dtype = jnp.float64, device=device_main)
           
        elif Gauss_num == 8:
            quad_weights= jnp.array([0.13621784, 0.11378216, 0.13621784, 0.11378216, 0.13621784,
                                    0.11378216, 0.13621784, 0.11378216], dtype = jnp.float64, device=device_main)
            quad_points = jnp.array([[0.3281633 , 0.10804725, 0.3281633 , 0.10804725, 0.3281633 ,
                                   ...
                                   [0.3281633 , 0.10804725, 0.01551009, 0.67585825, 0.3281633 ,
                                    0.10804725, 0.3281633 , 0.10804725]], dtype = jnp.float64, device=device_main).T
        elif Gauss_num == 14:
            quad_weights = jnp.array([0.07349304, 0.11268793, 0.07349304, 0.11268793, 0.07349304,
                                       ...
                                       0.04254602, 0.04254602, 0.04254602, 0.04254602], dtype = jnp.float64, device=device_main)
            quad_points = jnp.array([[0.09273525, 0.31088592, 0.09273525, 0.31088592, 0.09273525,
                                   ...
                                   [0.09273525, 0.31088592, 0.72179425, 0.06734224, 0.09273525,
                                    ...
                                    0.0455037 , 0.4544963 , 0.4544963 , 0.0455037 ]], dtype = jnp.float64, device=device_main).T
        elif Gauss_num == 24:
            quad_weights = jnp.array([0.03992275, 0.01007721, 0.05535718, 0.03992275, 0.01007721,
                                   ...
                                   0.04821429, 0.04821429, 0.04821429, 0.04821429], dtype = jnp.float64, device=device_main)
            quad_points = jnp.array([[0.21460287, 0.04067396, 0.32233789, 0.21460287, 0.04067396,
                                   ...
                                   [0.21460287, 0.04067396, 0.32233789, 0.35619139, 0.87797812,
                                    ...
                                    0.063661  , 0.26967233, 0.26967233, 0.063661  ]], dtype = jnp.float64, device=device_main).T
...
def get_Gauss_num(elem_type, dim):
    if elem_type.startswith('C3D8') or elem_type.startswith('C3D20'): # 3D Hex element
        ...
        Gauss_num_CFEM = 8  # 1, 4, 8, 14, 24
        Gauss_num_norm = 8
        Gauss_num_tr = 3    # 1, 3, 4, 6
